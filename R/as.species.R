...
#' movement in the direction specified. Slots x and y are only required
...
#' heavier than average birds. We assume that heavier birds will fly longer each day, so
#' specify `morphpar2sign`=`"Pos"`. If we assumed that heavier birds will fly less distance,
...
#' @slot morphpar1sign Do higher values of morphpar1 lead to longer or shorter distances traveled each day? Specify "Pos" if faster and "Neg" if slower.
...
#' @slot morphpar2sign Do higher values of morphpar2 lead to longer or shorter distances traveled each day? Specify "Pos" if faster and "Neg" if slower.
#' 
#' @examples
#' 
#' Example 1 -- Birds
